...
5. Fixed a broken `pyzmq` install on this machine that prevented any Jupyter kernel from starting at all (`Bad file descriptor (epoll.cpp:73)`) — unrelated to the notebook itself, but blocked every execution attempt until reinstalled.
